...
export class ProductService {
  ...
  async create({ createProductInput }) {
    ...
    // 구조분해 할당을 통해 input에서 categoryId 분리
    const { productSaleslocation, productCategoryId, ...product } =
      createProductInput;

    // const result0 = await this.productCategoryRepository.save({
    //   ...productCategory,
    // });
    ...
    const result0 = await this.productCategoryRepository.findOne({
