...
export class ProductService {
  ...
  constructor(
    ...
    // -> module.ts에도 ProductSaleslocation 엔티티 파일을 주입시켜야 함
  ...
  async create({ createProductInput }) {
    ...
    // 2 상품과 상품 거래 위치를 같이 등록하는 경우 + 카탈로그ID
    // 구조분해 할당을 통해 input에서 데이터 개별 분리
    // 아래 해당하는 데이터는 분리, 나머지는 rest 파라미터를 통해 product에 저장됨
    const { productSaleslocation, productCategoryId, productTags, ...product } =
      createProductInput;

    // ["#와", "#배가", "#너무", "#부르다"]
    const result2 = []; // [{name: ..., id: ...}, ...]
    // n개의 태그를 받아오기 위해 for문 + # 제거
    for (let i = 0; i < productTags.length; i++) {
      const tagname = productTags[i].replace('#', '');

      // 입력받은 tag가 DB에 이미 존재하는지 확인하기 위해 find
      const prevTag = await this.productTagRepository.findOne({
        // 조건: 입력받은 tagname과 동일한 tag가 DB에 있는가
        where: { name: tagname },
      });
    ...
    const result0 = await this.productSaleslocationRepository.save({
      // 스프레드 연산자를 사용해 저장
      // ...createProductInput.productSaleslocation
      ...productSaleslocation,
    });

    const result1 = await this.productCategoryRepository.findOne({
    ...
    const result = await this.productRepository.save({
      ...product,
      productSaleslocation: result0, // result를 그대로 넣음
      // 기존 tag와 새로운 tag 모두 productTag에 저장
      productTags: result2,
      // 데이터 저장시 catagoryId 함께 저장
      // productCategory: { id: productCategoryId, name: ctgName },
      productCategory: result1,
      // ********** 추가구현 **********
      // 상품 등록시 입력된 카테고리명 함께 전달
      // 1 의존성 부여
      // 2 카테고리 id를 이용해 해당 카테고리명 find
      // 3 스프레드 연산자를 통해 저장해 그대로 전달
      // ******************************
      // productSaleslocation: {
      // id: result.id
      // }, // id 만 넣음
...
// 중요!
// 의존성 주입은 service.ts와 module.ts 모두 해주어야 함