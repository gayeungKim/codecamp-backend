...
export class AuthService {
  ...
  setRefreshToken({ user, res }) {
    // sign: JwtService의 메서드. refreshToken 발급
    const refreshToken = this.jwtService.sign(
      { email: user.email, sub: user.id },
      // 이때, 항상 refreshToken의 expire은 accessToken의 expire
      // 보다 길어야 함
      { secret: 'myRefreshKey', expiresIn: '2w' },
    );

    // 개발 환경
    // 받아온 res의 header(res.setHeader())의 cookie 부분
    // ('Set-Cookie')에 refreshToken을 추가해야 함
    res.setHeader('Set-Cookie', `refreshToken=${refreshToken}; paty=/;`);
  ...
  getAccessToken({ user }) {
    ...
    return this.jwtService.sign(
      ...
      // 1s 1m 1h 1w
      { secret: 'myAccessKey', expiresIn: '1m' },
    );
  }
}
